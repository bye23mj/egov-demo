...
Atlassian 토큰은 **계정 단위**라 Jira와 공유한다. **MCP 우선** — 저장소 루트 `.mcp.json`의 `atlassian`(mcp-atlassian)이 Jira+Confluence를 함께 제공한다. REST 폴백 시 `.env`(`env_loader.load_env()`)에서 `CONFLUENCE_BASE_URL`(`https://nsonesoft.atlassian.net/wiki`), `CONFLUENCE_EMAIL`(없으면 `JIRA_EMAIL`), `CONFLUENCE_API_TOKEN`(없으면 `JIRA_TOKEN`)을 로드. 인증은 HTTP Basic `(email, token)`, 토큰은 `.env`(gitignore)에만 둔다.
...
## REST 폴백 엔드포인트 (Confluence Cloud v1)

베이스 `$CONFLUENCE_BASE_URL`, 인증 `-u "$CONFLUENCE_EMAIL:$CONFLUENCE_API_TOKEN"`.

| 작업 | 엔드포인트 |
|---|---|
| 스페이스 목록 | `GET /rest/api/space?limit=50` |
| CQL 검색 | `GET /rest/api/content/search?cql=...` |
| 페이지 생성 | `POST /rest/api/content` (body `storage` representation, `ancestors`로 부모 지정) |
| 첨부 업로드 | `POST /rest/api/content/{id}/child/attachment` (헤더 `X-Atlassian-Token: no-check`, `-F file=@...`) |
