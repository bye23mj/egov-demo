...
```text
1) 테이블스페이스  → 2) user/스키마(+권한)  → 3) 테이블  → 4) 인덱스  → 5) 제약/시퀀스
```
...
1. 산출물(컬럼/테이블정의서·migrations) 로드 → 대상 DBMS 분기 DDL 생성(`ddl/` 저장, drop 동반).
2. 의존 순서(테이블스페이스→user→테이블→인덱스)대로 실행한다.
3. 실행 결과(객체 생성/오류)를 점검하고 컬럼정의서와 대조 검증한다.
